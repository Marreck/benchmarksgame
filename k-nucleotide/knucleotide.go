 ...
 *
 * Based on https://benchmarksgame-team.pages.debian.net/benchmarksgame/program/knucleotide-go-6.html and https://benchmarksgame-team.pages.debian.net/benchmarksgame/program/knucleotide-gpp-2.html
 *
 * See https://github.com/Marreck/benchmarksgame for Go tests and benchmarks
 */
...
func calc32(data []byte, result map[uint32]*int, begin int, size int, wg *sync.WaitGroup) {
	...
	// Create a map to do the counts in. We don't use the map we are passed but instead return a copy of this
	...
	*p++
	...
	for idx := start; idx < end; idx += goroutineCount {
		...
		// For a low number of different keys using a map with pointers is faster than just using a value map and do: m[key]++
		p, ok := result[key]
		if !ok {
			p = new(int)
			result[key] = p
